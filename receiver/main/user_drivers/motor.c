...
void motor_config()
{
    ...
    io_config.pin_bit_mask = ((1 << GPIO_OUTPUT_AIN1) | (1 << GPIO_OUTPUT_AIN2) | (1 << GPIO_OUTPUT_BIN1) | (1 << GPIO_OUTPUT_BIN2));
...
void left_wheel_forward(int per)
{
    turn_forward();
    // stop();
    ledc_set_duty(LEDC_HIGH_SPEED_MODE, LEDC_CHANNEL_0, perDuty2Duty(per));
...
void right_wheel_forward(int per)
{
    turn_forward();
    // stop();
    ledc_set_duty(LEDC_HIGH_SPEED_MODE, LEDC_CHANNEL_0, perDuty2Duty(per));
...
void car_control(const char *cmd, int pwm)
{
